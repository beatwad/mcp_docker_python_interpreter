...
WORKING_DIR = Path("/app/data").absolute()
...
def get_python_environments() -> List[Dict[str, str]]:
    ...
    if DEFAULT_PYTHON_PATH != sys.executable:
        try:
            ...
            environments.append(
                {"name": "default", "path": DEFAULT_PYTHON_PATH, "version": version}
            )
            ...
            for env in conda_info.get("envs", []):
                ...
                if os.path.exists(python_path):
                    ...
                    try:
                        ...
                        environments.append(
                            {"name": env_name, "path": python_path, "version": version}
                        )
                    except Exception:
...
def execute_python_code(
    code: str, python_path: Optional[str] = None, working_dir: Optional[str] = None
) -> Dict[str, Any]:
    """
    ...
    try:
        result = subprocess.run(
            [python_path, temp_path], capture_output=True, text=True, cwd=working_dir
        )
...
def find_python_files(directory: Union[str, Path]) -> List[Dict[str, str]]:
...
@mcp.tool()
def write_file(
    file_path: str, content: str, overwrite: bool = False, encoding: str = "utf-8"
) -> str:
    """
...
def get_working_directory_listing() -> str:
    ...
    try:
        files = find_python_files(WORKING_DIR)
        return json.dumps(
            {"working_directory": str(WORKING_DIR), "files": files}, indent=2
        )
    except Exception as e:
...
def list_installed_packages(environment: str = "default") -> str:
    ...
    if environment == "default" and not any(
        e["name"] == "default" for e in environments
    ):
        environment = "system"
...
@mcp.tool()
def run_python_code(
    code: str, environment: str = "default", save_as: Optional[str] = None
) -> str:
    """
...
@mcp.tool()
def install_package(
    package_name: str, environment: str = "default", upgrade: bool = False
) -> str:
    """
    ...
    if path.is_absolute():
        if not is_path_allowed(path):
            security_status = (
                ...
                else "ENABLED, but this path is not allowed"
            )
...
@mcp.tool()
def run_python_file(
    file_path: str, environment: str = "default", arguments: Optional[List[str]] = None
) -> str:
    """
    ...
    try:
        ...
        result = subprocess.run(
            cmd, capture_output=True, text=True, check=False, cwd=WORKING_DIR
        )
